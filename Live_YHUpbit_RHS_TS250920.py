...
# - windows: Windows 등 GUI 환경에서 실행 시 'windows'로 변경 linux
RUN_MODE = 'windows'
...
TRAILING_STOP_PERCENT = 5  # 5% 트레일링 스탑
SCAN_INTERVAL_HOURS = 24  # 24시간 간격으로 스캔
DAILY_START_TIME = "03:23"  # 매일 오전 3시 23분에 시작
...
# 검색된 패턴 저장 디렉토리 생성
if not os.path.exists(SCANNED_PATTERNS_DIR):
    os.makedirs(SCANNED_PATTERNS_DIR)
...
class MACDAnalyzer:
    ...
    def is_macd_increasing(self, market):
        """1분봉 기준 MACD 증가 확인"""
        try:
            # 1분봉 데이터 가져오기 (최근 50개)
            ohlcv_data = self.upbit_api.get_ohlcv(market, count=50, timeframe="minutes/1")
            ...
            # 최근 3개의 MACD 값
            macd_prev_prev = macd.iloc[-3]
            macd_prev = macd.iloc[-2]
            macd_current = macd.iloc[-1]

            # MACD가 증가하는지 확인 (이전보다 현재 MACD 값이 큰 경우)
            # 3개 봉 모두 증가하는 추세인지 또는 최근 2개 봉이 증가하는지 확인
            is_increasing = (macd_current > macd_prev) or (macd_prev > macd_prev_prev and macd_current > macd_prev)

            # MACD 값도 함께 반환하여 로깅용으로 사용
            return is_increasing, macd_prev, macd_current

        except Exception as e:
            print(f"MACD 분석 중 오류 ({market}): {e}")
            return False, 0, 0
...
class StockScannerThread(QThread):
    ...
    def __init__(self, market_list, parent):
        ...
        self.macd_analyzer = MACDAnalyzer(self.upbit_api)
        self.investment_amount = 0  # 이번 스캔 주기의 투자 금액
        self.found_patterns = []  # 검색된 패턴 저장
            ...
            try:
                ...
                if self.parent.analyzeStock(market['market'], days=250):
                    ...
                    if market_info:
                        ...
                        # MACD 조건 확인 (1분봉 기준 증가)
                        macd_increasing, macd_prev, macd_current = self.macd_analyzer.is_macd_increasing(
                            market['market'])

                        if macd_increasing:
                            print(f"{market['market']}: MACD 증가 조건 충족 (이전: {macd_prev:.6f}, 현재: {macd_current:.6f})")
                            self._execute_buy_order(market_info)
                        else:
                            print(
                                f"{market['market']}: 패턴은 발견되었지만 MACD 증가 조건 미충족 (이전: {macd_prev:.6f}, 현재: {macd_current:.6f})")
            except Exception as e:
                print(f"마켓 {market['market']} 분석 중 오류: {e}")

        # 검색된 패턴 저장
        self.found_patterns = found_patterns
        self._save_scanned_patterns(found_patterns)
    ...
    def _save_scanned_patterns(self, found_patterns):
        """검색된 패턴을 파일로 저장"""
        try:
            ...
            filename = f"{SCANNED_PATTERNS_DIR}/{timestamp}_patterns.csv"

            with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['timestamp', 'market', 'name', 'price', 'change_rate', 'volume', 'pattern_detected',
                              'yearly_low']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writeheader()

                for pattern in found_patterns:
                    pattern_detected = self.parent.pattern_recognizer.detect_inverse_head_shoulders(pattern['code'])
                    yearly_low = self.parent.pattern_recognizer.is_near_yearly_low(pattern['code'])
                    ...
                    writer.writerow({
                        ...
                        'change_rate': pattern['diff_rate'],
                        'volume': pattern['volume'],
                        'pattern_detected': pattern_detected,
                        'yearly_low': yearly_low
                    })

            print(f"검색된 패턴 저장 완료: {filename}")
            self.parent.add_to_log(f"검색된 패턴 저장 완료: {filename}")

        except Exception as e:
            print(f"패턴 저장 중 오류: {e}")
            self.parent.add_to_log(f"패턴 저장 오류: {e}")
