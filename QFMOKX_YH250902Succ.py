...
CONTRACT_AMOUNT = 0.2  # 계약 수량 (⚠️ 실거래이므로 신중하게 설정)
...
TP_PARTIAL_1 = 10.0  # 부분 익절 1차 (+30%)
TP_PARTIAL_2 = 20.0  # 부분 익절 2차 (+50%)
TP_FULL = 30.0  # 전체 익절 기준 (+100%)
...
def place_order(signal, amount=CONTRACT_AMOUNT):
    try:
        ...
        order_params = {
            ...
            "side": side,
            "amount": amount,
            "params": {"tdMode": "cross", "posSide": posSide}#isolated
        }
...
def close_partial_position(position, close_percent=0.5):
    ...
    try:
        ...
        print(f"🔓 부분 청산: {position['side']} / {close_amount} 계약 ({close_percent * 100}%)")
...
def close_full_position(position):
    ...
    try:
        ...
        print(f"🔒 전체 청산: {position['side']} / {position['size']} 계약")
...
def manage_take_profit(position, signal):
    ...
    if current_roi >= TP_FULL:
        print(f"🎯 전체 익절: {current_roi:.2f}% ≥ {TP_FULL}%")
        return close_full_position(position)
