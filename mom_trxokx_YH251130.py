...
LEVERAGE = 10  # TRXUSDT Perp.에 적용할 레버리지
TD_MODE = "cross"  # 교차 마진 모드
...
def set_leverage(symbol, leverage=LEVERAGE, mgnMode=TD_MODE):
    """레버리지를 설정합니다."""
    url_path = "/api/v5/account/set-leverage"
    body = {
        "instId": symbol,
        "lever": str(leverage),
        "mgnMode": mgnMode
    ...
    try:
        ...
        logger.error(f"Exception occurred while setting leverage: {e}")
        return False
...
def status_check():
    ...
    return jsonify(
        {"status": "Flask server with AI-check is running", "symbol": "TRXUSDT.P", "leverage": LEVERAGE}), 200
...
def webhook_receiver():
    ...
    # TRXUSDT Perp.에 대한 레버리지 설정
    leverage_success = set_leverage(okx_symbol, LEVERAGE, TD_MODE)
    if not leverage_success:
        logger.warning(f"Failed to set leverage for {okx_symbol}, but continuing...")
    ...
    return jsonify({"status": "processed", "symbol": okx_symbol, "leverage": LEVERAGE}), 200
